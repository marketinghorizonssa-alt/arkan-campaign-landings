...
$backupDir="$root/_backup_form_tracking_20260917";
@mkdir($backupDir,0755,true);
if (is_file($app)) @copy($app,"$backupDir/app.js.bak");
...
const phoneInfo=v=>{
  ...
  return {raw,normalized:p,digits};
};
...
document.querySelectorAll('form[data-lead-form]').forEach(form=>form.addEventListener('submit',async e=>{
 ...
 finally{form.dataset.submitting='0';btn.disabled=false;btn.textContent=old}
}));
})();
JS;
...
foreach($it as $f){
  ...
  if(strpos($s,'data-lead-form')===false) continue;
  @copy($path,$backupDir.'/'.str_replace(['/','\\'],['_','_'],substr($path,strlen($root)+1)).'.bak');
  $n=preg_replace('/placeholder="05xxxxxxxx"/u','placeholder="مثال: 055... أو +966... أو رقم دولي"',$s);
  $n=preg_replace('#/assets/app\.js\?v=[^"\']+#','/assets/app.js?v=20260917-form-v3',$n);
  if($n!==$s){file_put_contents($path,$n);$changed++;}
}
file_put_contents("$root/.form-tracking-version","CORTS_FORM_TRACKING_V3_20260917\n");
