...
function formHtml(string $pageId): string {
    return '<aside class="form-card" id="form"><h2>ابدأ تقييم حالتك مجانًا</h2><form id="leadForm" novalidate>'
        ...
        . '<div class="field"><label for="phone">رقم الجوال</label><input id="phone" name="phone" class="ltr-input" type="tel" inputmode="tel" autocomplete="tel" required pattern="(?:\+?966|0)?5\d{8}" placeholder="05xxxxxxxx" dir="ltr"></div>'
        ...
        . '<div class="two"><div class="field"><label for="property_type">نوع العقار</label><select id="property_type" name="property_type" required><option value="">اختر</option><option>وحدة جاهزة</option><option>بناء ذاتي</option><option>رهن عقاري</option></select></div><div class="field"><label for="employer_type">جهة العمل</label><select id="employer_type" name="employer_type" required><option value="">اختر</option><option>حكومي مدني</option><option>حكومي عسكري</option><option>شبه حكومي</option><option>قطاع خاص</option><option>متقاعد</option></select></div></div>'
        . '<label class="consent"><input type="checkbox" name="privacy_consent" value="1" checked required><span>أوافق على <a href="/سياسة-الخصوصية/" target="_blank" rel="noopener">سياسة الخصوصية</a> والتواصل معي بخصوص الطلب.</span></label><button class="btn btn-primary" type="submit">إرسال طلب التقييم</button><div class="form-status" id="formStatus" role="status" aria-live="polite"></div></form>'
        . '<div class="quick-contact"><a class="track-call" href="tel:' . PHONE_E164 . '">' . icon('phone') . ' اتصال مباشر</a><a class="track-whatsapp" href="https://wa.me/' . WHATSAPP_NUMBER . '" target="_blank" rel="noopener">' . icon('whatsapp') . ' واتساب</a></div></aside>';
}
...
function pageHtml(string $path, array $p, string $leadEndpoint): string {
    ...
    $body = '<body class="' . e($p['theme']) . '">' . headerHtml()
        ...
        . '<section class="section section-ice"><div class="container"><div class="section-head"><span class="tag light-tag">خطوات واضحة</span><h2 class="top-gap">من الطلب إلى الخطوة التالية</h2><p>أرسل بياناتك الأساسية، وسيتواصل معك الفريق لفهم حالتك وتوضيح الخيارات المتاحة.</p></div><div class="steps"><article class="step"><h3>أرسل البيانات الأساسية</h3><p>الاسم والجوال والمدينة ونوع العقار وجهة العمل.</p></article><article class="step"><h3>نراجع حالتك</h3><p>نفهم الهدف والمشكلة ونحدد المعلومات الإضافية المطلوبة عند الحاجة.</p></article><article class="step"><h3>نوضح المسار المناسب</h3><p>نتواصل معك لشرح الخيارات والخطوة التالية بصورة واضحة.</p></article></div></div></section>'
        ...
        . '<section class="section section-mist"><div class="container faq"><div class="section-head"><h2>الأسئلة الشائعة</h2><p>إجابات مباشرة قبل إرسال طلب التقييم.</p></div><details><summary>هل أركان بنك أو جهة تمويل؟</summary><p>لا. أركان التنفيذية تقدم حلولًا واستشارات مالية وعقارية ولا تمنح قرضًا مباشرًا.</p></details><details><summary>هل يمكن ضمان قبول التمويل؟</summary><p>لا يمكن ضمان موافقة أي جهة. الهدف هو دراسة الحالة وتوضيح المسارات التي يمكن بحثها وفق المتطلبات.</p></details><details><summary>هل الاستشارة الأولية مجانية؟</summary><p>نعم، التقييم الأولي للبيانات الأساسية مجاني، وقد نحتاج معلومات إضافية لفهم الحالة بصورة أدق.</p></details><details><summary>لماذا جهة العمل محددة بخيارات؟</summary><p>النموذج مخصص للفئات المعتمدة حاليًا: حكومي مدني، حكومي عسكري، شبه حكومي، قطاع خاص، ومتقاعد.</p></details></div></section>'
        . '<section class="contact-band"><div class="container contact-grid"><div><h2>تحتاج تواصلًا مباشرًا؟</h2><p>اتصل على ' . phoneHtml() . ' أو ابدأ المحادثة عبر واتساب.</p></div><div class="contact-actions"><a class="btn btn-ghost track-call" href="tel:' . PHONE_E164 . '">' . icon('phone') . 'اتصل الآن</a><a class="btn btn-primary track-whatsapp" href="' . e($wa) . '" target="_blank" rel="noopener">' . icon('whatsapp') . 'واتساب</a></div></div></section></main>'
...
function thankYouHtml(): string {
    $body = '<body><main class="thank-page"><section class="thank-card">' . logoHtml() . '<h1>تم استلام طلبك</h1><p>تم تسجيل بياناتك بنجاح، ويمكنك الآن متابعة الطلب مباشرة عبر واتساب.</p><div class="thank-actions"><a id="afterFormWa" class="btn btn-primary" href="https://wa.me/' . WHATSAPP_NUMBER . '" target="_blank" rel="noopener">' . icon('whatsapp') . 'متابعة عبر واتساب</a><a class="btn btn-ghost" href="/">العودة للرئيسية</a></div></section></main><script src="/assets/thank-you.js?v=4" defer></script></body>';
