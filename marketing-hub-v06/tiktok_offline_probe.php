...
$pick=null;$id=[];foreach($records as $lid=>$r){if(!is_array($r))continue;if(strtolower(s($r['source']??''))!=='tiktok')continue;if(strtolower(s($r['stage']??''))!=='interested')continue;$lead=s($r['lead_id']??$lid);$x=$ids[$lead]??[];$p=phone(s($x['phone']??''));if($p==='')continue;$pick=$r;$id=$x;break;}
if(!$pick){echo json_encode(['ok'=>false,'error'=>'no_interested_tiktok_lead'])."\n";exit(2);}
$p=phone(s($id['phone']??''));$lead=s($pick['lead_id']??'');$when=s($pick['last_seen_at']??'');$ts=$when!==''?gmdate('c',strtotime($when)?:time()):gmdate('c');$eventId='offprobe_button_'.substr(hash('sha256',$offlineSet.'|'.$lead.'|interested_button'),0,30);
$payload=['event_set_id'=>$offlineSet,'event'=>'BUTTON','event_id'=>$eventId,'timestamp'=>$ts,'context'=>['user'=>['phone_numbers'=>[hash('sha256','+'.$p)]]],'properties'=>['event_channel'=>'other','lead_stage'=>'interested','source'=>'tiktok']];
$ch=curl_init('https://business-api.tiktok.com/open_api/v1.3/offline/track/');curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_POST=>true,CURLOPT_HTTPHEADER=>['Access-Token: '.$token,'Content-Type: application/json','Accept: application/json'],CURLOPT_POSTFIELDS=>json_encode($payload,JSON_UNESCAPED_SLASHES),CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_TIMEOUT=>20,CURLOPT_FOLLOWLOCATION=>false]);$body=curl_exec($ch);$errno=curl_errno($ch);$err=curl_error($ch);$http=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);curl_close($ch);$dec=is_string($body)?json_decode($body,true):null;$code=is_array($dec)?($dec['code']??null):null;$msg=is_array($dec)?($dec['message']??null):null;$ok=$errno===0&&$http>=200&&$http<300&&(int)$code===0;echo json_encode(['ok'=>$ok,'http'=>$http,'api_code'=>$code,'message'=>$msg,'curl_error'=>$errno?$err:null,'event_set_id'=>$offlineSet,'event'=>'BUTTON','lead_stage'=>'interested'],JSON_UNESCAPED_SLASHES)."\n";
