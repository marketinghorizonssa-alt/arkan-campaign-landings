...
if ($action === 'summary') {
    ...
    $wabas = []; $numbers = [];
    foreach ($c as $v) {
        if (!empty($v['waba_id'])) $wabas[$v['waba_id']] = 1;
        if (!empty($v['business_number'])) $numbers[$v['business_number']] = 1;
    }
    $today = gmdate('Y-m-d');
    $count = 0;
...
if ($action === 'tag' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_array($body)) { http_response_code(400); echo json_encode(['ok'=>false,'error'=>'invalid_json']); exit; }
    ...
    $delivery = null;
    if ($changed && $tag !== 'lost') {
        $eventId = 'mev_' . bin2hex(random_bytes(8));
        $event = [
            'id' => $eventId,
            'event' => $tag,
            'conversation_id' => $id,
