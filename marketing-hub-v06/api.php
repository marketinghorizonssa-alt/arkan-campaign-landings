...
if($action==='tag'&&$_SERVER['REQUEST_METHOD']==='POST'){
    $body=body_json();$id=(string)($body['conversation_id']??'');$tag=strtolower(trim((string)($body['tag']??'')));$allowed=['message_received','interested','qualified','converted','purchased','lost'];if($id===''||!in_array($tag,$allowed,true)){http_response_code(422);echo json_encode(['ok'=>false,'error'=>'invalid_input']);exit;}
    $c=load_json($convFile,[]);if(!isset($c[$id])){http_response_code(404);echo json_encode(['ok'=>false,'error'=>'conversation_not_found']);exit;}$changed=(($c[$id]['current_tag']??null)!==$tag);$now=gmdate('c');$c[$id]['current_tag']=$tag;$c[$id]['tagged_at']=$now;$c[$id]['tag_source']='manual';$c[$id]['auto_label_reason']='';$hist=is_array($c[$id]['label_history']??null)?$c[$id]['label_history']:[];if($changed){$hist[]=['tag'=>$tag,'source'=>'manual','reason'=>'manual_override','confidence'=>1.0,'at'=>$now];if(count($hist)>20)$hist=array_slice($hist,-20);}$c[$id]['label_history']=$hist;save_json($convFile,$c);$delivery=null;
    if($changed&&$tag!=='lost'){
