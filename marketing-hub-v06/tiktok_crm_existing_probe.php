...
function sendEvent(string $token,string $setId,string $eventName,array $record,array $identity):array{
  ...
  $eid='probe_'.substr(hash('sha256',$setId.'|'.s($record['lead_id']??'').'|'.$stage.'|'.$occur),0,32);
  $payload=['event_source'=>'crm','event_source_id'=>$setId,'data'=>[['event'=>$eventName,'event_time'=>$ts,'event_id'=>$eid,'user'=>$user,'properties'=>['lead_stage'=>$stage,'origin_source'=>'tiktok','probe'=>'existing_lead']]]];
...
$map=['interested'=>'Interested','qualified'=>'Qualified','unqualified'=>'Unqualified'];$picked=[];foreach($records as $lid=>$r){if(!is_array($r)||strtolower(s($r['source']??''))!=='tiktok')continue;$stage=strtolower(s($r['stage']??''));if(!isset($map[$stage])||isset($picked[$stage]))continue;$id=$ids[s($r['lead_id']??$lid)]??[];if(phone(s($id['phone']??''))==='')continue;$picked[$stage]=[$r,$id];}
$out=['ok'=>true,'event_set_id'=>$setId,'attempted'=>0,'accepted'=>0,'rejected'=>0,'stages'=>[]];foreach($map as $stage=>$eventName){if(!isset($picked[$stage])){$out['stages'][$stage]=['status'=>'no_eligible_record'];continue;}[$r,$id]=$picked[$stage];$resp=sendEvent($token,$setId,$eventName,$r,$id);$out['attempted']++;if($resp['ok'])$out['accepted']++;else{$out['rejected']++;$out['ok']=false;}$out['stages'][$stage]=['status'=>$resp['ok']?'accepted':'rejected','http'=>$resp['http'],'api_code'=>$resp['code'],'message'=>$resp['message']];}
