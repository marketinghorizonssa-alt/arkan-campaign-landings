...
$map=['interested'=>'Interested','qualified'=>'Qualified','unqualified'=>'Unqualified'];$picked=[];foreach($records as $lid=>$r){if(!is_array($r)||strtolower(s($r['source']??''))!=='tiktok')continue;$stage=strtolower(s($r['stage']??''));if(!isset($map[$stage])||isset($picked[$stage]))continue;if(s($r['native_ids']['ttclid']??'')==='')continue;$id=$ids[s($r['lead_id']??$lid)]??[];if(phone(s($id['phone']??''))==='')continue;$picked[$stage]=[$r,$id];}
